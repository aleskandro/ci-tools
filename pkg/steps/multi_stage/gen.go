...
func addCliInjector(imagestream string, pod *coreapi.Pod) {
	...
	pod.Spec.InitContainers = append(pod.Spec.InitContainers, coreapi.Container{
		...
		Args: []string{"-c", fmt.Sprintf("ARCH=$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/'); if [[ -e /usr/share/openshift/linux_${ARCH}/oc.rhel8 ]]; then /bin/cp /usr/share/openshift/linux_${ARCH}/oc.rhel8 %s; else /bin/cp /usr/bin/oc %s; fi", filepath.Join(CliMountPath, "oc"), CliMountPath)},
		VolumeMounts: []coreapi.VolumeMount{{
			Name:      volumeName,
